...
#################################################
###### Create Hospital free days at day 90 ###### 
#################################################
h <- 90 # change horizon here if needed

dat$horizon <- dat$charttime.kdigo3 + h *3600*24 # convert 90 days in seconds
...
temp <- merge(x = admissions %>% rename(subject_id = SUBJECT_ID) %>% filter(subject_id %in% dat$subject_id), y=dat, by="subject_id", all.x= TRUE) %>%
        mutate(readmission = ADMITTIME < horizon & ADMITTIME > dischtime) %>% # boolean for readmission between discharge and horizon (day 90)
...
exp_dat_mice <- mice(exp_dat, m=5, seed = 1, predictorMatrix = predM, method = meth)
